...
import com.glitchcam.vepromei.edit.data.PropsList;
...
public class AssetFxUtil {
     ...
     * Get filter data
     * */
    public static ArrayList<FilterItem> getFilterData(Context context, ArrayList<NvAsset> filterAssetList,
          List<String> builtinVideoFxList, boolean isAddCartoon, boolean isFitRatio) {
        
        ArrayList<FilterItem> filterList = new ArrayList<>();

        FilterItem filterItem = new FilterItem();
        filterItem.setFilterName(context.getString(R.string.filter_none));
        filterItem.setImageId(R.mipmap.filter_null);
        filterItem.setBackgroundColor(context.getColor(R.color.douyin_speed_color));
     ...
     * Get glitch data
     * */
    public static ArrayList<FilterItem> getGlitchFilterData(Context context, ArrayList<NvAsset> filterAssetList,
                                                            List<String> builtinVideoFxList, boolean isAddCartoon, boolean isFitRatio) {
        ...
        String bundlePath = "glitch/info.txt";
        Util.getBundleFilterInfo(context, filterAssetList, bundlePath);
        ...
        for (NvAsset asset : filterAssetList) {
            ...
            FilterItem newFilterItem = new FilterItem();
            if (asset.isReserved()) {
                String coverPath = "file:///android_asset/glitch/";
                coverPath += asset.uuid;
                coverPath += ".png";
                /*
                 * 加载assets/glitch文件夹下的图片
                 * Load images in the assets / glitch folder
                 * */
                asset.coverUrl = coverPath;
            }
            newFilterItem.setFilterMode(FilterItem.FILTERMODE_PACKAGE);
