...
//                        updateCaptionBoundingRect();
                        ...
                        if (keyFrameMap != null && keyFrameMap.size() > 0) {
                            // 不断的创建关键帧，去获取当前的位置字幕的边缘坐标，更新完字幕框之后，删除当前位置的关键帧
        ...
        if (captionInfo != null) {
            ...
            for (Long aLong : keyFrameMap.keySet()) {
                if (timelineCurrentPosition >= aLong - 100000 && timelineCurrentPosition <= aLong + 100000) {
                    ...
                    boolean aTRANS_X = mCurCaption.removeKeyframeAtTime(TRANS_X, aLong - mCurCaption.getInPoint());
                    boolean aTRANS_Y = mCurCaption.removeKeyframeAtTime(TRANS_Y, aLong - mCurCaption.getInPoint());
                    boolean aSCALE_X = mCurCaption.removeKeyframeAtTime(SCALE_X, aLong - mCurCaption.getInPoint());
                    boolean aSCALE_Y = mCurCaption.removeKeyframeAtTime(SCALE_Y, aLong - mCurCaption.getInPoint());
                    boolean aROTATION_Z = mCurCaption.removeKeyframeAtTime(ROTATION_Z, aLong - mCurCaption.getInPoint());
                    Log.d(TAG, "aTRANS_X=" + aTRANS_X + " aTRANS_Y=" + aTRANS_Y + " aSCALE_X=" + aSCALE_X + " aSCALE_Y=" + aSCALE_Y + " aROTATION_Z=" + aROTATION_Z);
                    break;
                }
            }
        }
                ...
                mOkBtn.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        if (mCurCaption != null) {
                            ...
                            updateCaptionBoundingRect();
                            mCurCaption.removeKeyframeAtTime(TRANS_X, frameDuration);
                            mCurCaption.removeKeyframeAtTime(TRANS_Y, frameDuration);
                            mCurCaption.removeKeyframeAtTime(SCALE_X, frameDuration);
                            mCurCaption.removeKeyframeAtTime(SCALE_Y, frameDuration);
                            mCurCaption.removeKeyframeAtTime(ROTATION_Z, frameDuration);
                        }
                        mVideoFragment.updateCaptionCoordinate(mCurCaption);
                        mVideoFragment.changeCaptionRectVisible();
                        seekTimeline(mStreamingContext.getTimelineCurrentPosition(mTimeline));
